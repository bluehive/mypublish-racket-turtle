...
**まず「契約（やくそく）」を1行で決めましょう。** この本の `tree` の契約は次のとおりです:

> **契約**: `tree` を呼び出したあと、カメは**位置も向きも呼び出し前と同じ**に戻る。

この契約を守るからこそ、「幹を描いたら枝を2本生やし、最後に元へ戻る」という再帰が、何段階でも正しく積み重ねられるのです。`forward (- size)` は「パッチ」ではなく、**この契約を満たすための仕様の一部**だと考えましょう。

では手順です。

1. まず幹（直線）を長さ `size` だけ前進して描きます。
2. 左へ角度 `angle` だけ向きを変え、深さ `(sub1 depth)`・長さ `(* size 0.7)` で **「小さな木」を再帰呼び出し** します（小木は契約どおり、呼び出し前の位置・向きに戻ってくる）。
3. 次に右へ `(* 2 angle)` だけ向きを変え、同じように **「小さな木」を再帰呼び出し** します。
4. 描き終わったら元の向きと位置に戻ります（契約の履行）。
