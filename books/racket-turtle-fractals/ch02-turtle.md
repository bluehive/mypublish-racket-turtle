...
> **この章のゴール**  
> カメ（タートル）を動かす命令のリスト（`CommandList`）を自分自身で設計し、正方形・正多角形・カラフルな図形を描けるようになる。「指示書データを作ってから描く」考え方（データ駆動）を理解する。  
...
多くのお絵かきツールでは、「今すぐ10歩進む」と指示した瞬間にキャラクターが動きます。つまり **「その場で直接動かす」** 描き方です（専門用語では手続き型とも呼びます）。
