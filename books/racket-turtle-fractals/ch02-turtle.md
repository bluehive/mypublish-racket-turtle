...
パパート教授は、「子どもたちが『もし自分が画面のカメになったら、どう動けばいいだろう？』と「自分がカメになったつもりで歩いてみる」という身体の感覚を使って楽しく幾何学とプログラミングを学べるように」という願いを込めてタートルを作りました。この思想は、後に LEGO マインドストームや Scratch のペン機能へ受け継がれています。

##### 💡 Racket ならではの考え方: 「命令を実行する」のではなく「指示書データを作る」
Scratch のペン機能では、`[10 歩動かす]` ブロックを置くと、その瞬間にキャラクターが画面上を動きます。つまり **「その場で直接動かす」** 描き方です（専門用語では手続き型とも呼びます）。

一方、Racket ではひと味違います。いきなりカメを動かすのではなく、**「カメに対する移動命令のリスト（指示書データ）」** を先にプログラミングで作ります。そして最後に `(draw 命令リスト)` を呼んで、一気にカメを走らせます。

このように、先に「指示書リスト（データ）」を作ってから `(draw …)` で一気に動かす考え方を、本書では **データ駆動（Data-Driven）** と呼びます。
