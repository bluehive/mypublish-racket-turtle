...
##### 🔁 「回数のくり返し」と「再帰」の違い
第2章で使った `repeat`（繰り返し）は、「同じ動きをそのまま回数分くり返す」のには便利でした。

しかし、次のような図形を描きたいときはどうでしょう？
- 1周進むごとに、**前進する距離が少しずつ長くなる螺旋（渦巻き）**
- 1段階進むごとに、**枝が半分に小さくなって2本に増える木**

決まった回数だけ同じ動作をくり返す書き方だと、「毎回長さが変わる」「分岐して2本に増える」といった変化を追うために、変数や場合分けが増えて一気に複雑になります。

Racket の **再帰** を使えば、**「自分自身を呼び出す」** という形で問題を少しずつ小さくでき、変化や拡大・縮小も短いコードで表現しやすくなります！
...
###### 🧮 階乗（Factorial: $n!$、例: $5! = 5×4×3×2×1 = 120$）の例
...
タートルグラフィックスで再帰を使うとき、よく使うパターンは次の2つです。
...
> **いまはパターン1（`append` 版の `spiral`）だけ動けば十分です。** `cons` 版は「リストの中にリスト」が入ります。`draw` がうまくならしてくれることが多いですが、仕組みとしては発展扱いです。
