...
実はこの2つ、「難しさ」がまったく違います。

**螺旋は、Scratch でもカンタンです。** 変数 `長さ` を1つ用意し、`[ (10) 回繰り返す ]` の中に「`長さ` 歩動かす → 90度回す → `長さ` を 2 ずつ変える」と入れるだけ。変数は1個で、パニックになる要素はありません。

**木こそが本当の難所です。** 枝が2本に分かれ、その先からさらに2本……と描くには「今どこにいるか」を覚えておき、枝を描き終えたら**元の場所へ戻る**必要があります。`[ (10) 回繰り返す ]` だけでは、分岐のたびに位置をリストで管理する（スタック※）か、深さの分だけブロックをコピー＆ペーストするしかなく、たちまち複雑になってしまいます。
※スタック＝「後で戻る場所」を積み上げておく入れ物のこと。

ここで登場するのが **再帰** です。じつは Scratch にも「**定義（Make a Block）**」ブロックがあり、その中で**自分自身を呼び出す**ことで再帰ができます。木も、定義ブロック1つ（10ブロック前後）で描けてしまいます。

ただし、Scratch の再帰には次の制約があります。
- 戻り値（計算結果を返すこと）がない
- ブロック内だけのローカル変数は作れない（定義ブロックの引数だけは呼び出しごとに独立）
- 深い再帰は描画が重くなる（「画面を再描画せずに実行する」にチェックを入れると高速化）
- スタック上限があり、あまり深くすると動かなくなる

Racket の **再帰** は、戻り値もローカルな引数も自由に使え、描画は最後にまとめて行うので、**「自分自身を呼び出す」** というたった数行のコードで、どんなに複雑な変化や拡大・縮小も表現できます。Scratch の「定義」を使ったことがある人なら、もう再帰の入り口に立っています。この章で、その仕組みを Racket でしっかりマスターしましょう！
